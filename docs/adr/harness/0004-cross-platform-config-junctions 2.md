...
2. **Native Directory Symbolic Links & Auto-Elevation (`setup-global-harness.sh`)**: In MINGW/MSYS shell environments, we will convert UNIX paths using `cygpath -w` (with trailing carriage return `\r` stripping via `tr -d '\r'`) and invoke native Windows symbolic linking (`cmd.exe /c mklink /d` for directories, `cmd.exe /c mklink` for files). Since Google Drive is mounted as a `vfat` volume on Windows, directory junctions (`mklink /j`) are rejected by the Windows kernel. To achieve zero-touch automation on fresh Windows devices, the script performs a non-elevated pre-flight check to test symlink capability. If this check fails (Developer Mode disabled / standard user), the script builds a temporary batch script that auto-enables Windows Developer Mode via the registry (`HKLM\...\AllowDevelopmentWithoutDevLicense = 1`) and creates all 6 symbolic links in a single elevated session via PowerShell's `Start-Process cmd -Verb RunAs -Wait` (requiring only a single, unavoidable UAC prompt). Once Developer Mode is enabled, subsequent runs require zero elevation. We fall back gracefully to junctions or `ln -s` on Unix.
