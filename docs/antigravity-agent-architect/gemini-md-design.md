...
GEMINI.md (and the cross-tool AGENTS.md) is operational policy for an AI agent, not a README for humans. The agent does not need to understand _why_ you use conventional commits. It needs to know the exact command to run and what “done” looks like.
...
`./GEMINI.md` or `./.agents/GEMINI.md` | Project | Via git | Team context  
`~/.gemini/antigravity-cli/GEMINI.md` | User | All projects | Personal preferences  
`./GEMINI.local.md` | Project-local | Never | Personal project notes  
`.agents/skills/<name>/SKILL.md` | Project rules/skills | Via git | Reusable knowledge modules  
`~/.gemini/antigravity-cli/skills/` | User rules/skills | All projects | Personal skills  
  
Skills files load automatically and provide structured context without cluttering GEMINI.md.
...
Reference other files within GEMINI.md:

```
See @README.md for project overview
Coding standards: @docs/STYLE_GUIDE.md
API documentation: @docs/API.md
Personal preferences: @~/.gemini/antigravity-cli/preferences.md
``` 

Import syntax: relative (`@docs/file.md`), absolute (`@/absolute/path.md`), or home directory (`@~/.gemini/antigravity-cli/file.md`). Maximum depth: 5 levels of imports.
...
Antigravity CLI | GEMINI.md | Yes (coexists with GEMINI.md)  
...
Antigravity CLI harness layer | Codex equivalent | Migration rule  
---|---|---  
`GEMINI.md` / `.agents/skills/` | `AGENTS.md` / nested `AGENTS.override.md` | Keep commands and completion rules canonical; split only when directory scope genuinely differs  
`.agents/skills/<name>/SKILL.md` | `.agents/skills/<name>/SKILL.md` or plugin skill | Port reusable workflows, but rewrite descriptions for Codex’s activation wording and budget  
`.agents/hooks.json` hooks | Codex `config.toml`, plugin hooks, or managed requirements hooks | Port deterministic gates first; test each hook with real tool events before enabling broadly  
Dynamic subagents | `~/.codex/agents/*.toml`, `.codex/agents/*.toml`, or built-in `worker` / `explorer` | Dynamic, runtime prompt-driven subagents in Antigravity CLI vs explicit subagent delegation in Codex  
Plugins | Codex plugins | Use plugins as the distribution unit after local hooks and skills are proven  
  
The important difference: Antigravity subagents are spawned dynamically at runtime via prompts and configuration artifacts, whereas Codex subagent workflows are explicit. That makes skills and hooks the right default for always-on harness behavior; subagents are for deliberate parallel work, review, and exploration.
...
agy --print "What instructions are you following for this project?"

# Verify specific rules are active
agy --print "What is your definition of done?"
``` 
