...
def main() -> None:
    ...
    for s, d in [("gitignore.template", ".gitignore"), ("skillsrc.template", ".skillsrc")]:
    ...
    # Physically copy the 10 global skills to .agents/skills/
    skills = ["diagnose", "improve-codebase-architecture", "setup-matt-pocock-skills", "tdd", "to-issues", "to-prd", "zoom-out", "caveman", "handoff", "write-a-skill"]
    for s in skills:
        src = os.path.join(root, "global/skills", s)
    ...
    subprocess.run(["git", "config", "--local", "user.email", "agent@alpha-zero-g.local"], cwd=dest)
    print("git config --local user.name \"Alpha Zero G Initializer\"")
    subprocess.run(["git", "config", "--local", "user.name", "Alpha Zero G Initializer"], cwd=dest)
    print("git add .")
